...
class GUI:
    ...
    def __init__(self, size:int, terminalStates:list, screenHeight: int = 500):
        ...
        self.gridBorder = 2
        self.separation = 10
        self.screen = pygame.display.set_mode((self.screenWidth+self.separation, self.screenHeight))
        ...
        for i in range(size):
            for j in range(size):
                ...
                if(self.env.coordinatesToIndex((i, j)) in self.env.terminalStates):
                    diff = np.array([255-terminalColor[0], 255-terminalColor[1], 255-terminalColor[2]], dtype = np.float32)
                    diff *= 0.75
                    ...
                    pygame.draw.rect(self.screen, background, (topLeft[0] + i*width + border, topLeft[1] + j*height + border, width-2*border, height-2*border), 0)
                    
                    # calculate a lighter version of the color
                    
                pygame.draw.rect(self.screen, color, (topLeft[0] + i*width, topLeft[1] + j*height, width, height), border)
    ...
    def drawValueFunction(self):
        ...
        for i in range(self.size):
            for j in range(self.size):
                ...
                self.screen.blit(text, (i*self.screenHeight/self.size + self.screenHeight/(2*self.size) - textRect.width/2, j*self.screenHeight/self.size + self.screenHeight/(2*self.size) - textRect.height/2))
    ...
    def drawGreedyPolicy(self):
        ...
        self.drawGrid((int(self.screenWidth/2) + self.separation, 0), (self.screenWidth + self.separation, self.screenHeight), self.size)
        ...
        for i in range(self.size):
            for j in range(self.size):
                ...
                for move in bestMoves:
                    ...
                    center = (int(self.screenWidth/2) + self.separation + i*self.screenHeight/self.size + self.screenHeight/(2*self.size), j*self.screenHeight/self.size + self.screenHeight/(2*self.size))
                    # calculate the coordinates of the end of the arrow
                    if(move == 0):
                        end = (center[0], center[1] - self.screenHeight/(2*self.size))
                    elif(move == 1):
                        end = (center[0] + self.screenHeight/(2*self.size), center[1])
                    elif(move == 2):
                        end = (center[0], center[1] + self.screenHeight/(2*self.size))
                    elif(move == 3):
                        end = (center[0] - self.screenHeight/(2*self.size), center[1])
        ...
        while running:
            for event in pygame.event.get():
                ...
                if event.type == pygame.KEYDOWN and event.key == pygame.K_SPACE:
                    for _ in range(10):
                        self.valueFunction, difference = iteration(policy, self.valueFunction, self.env)
...
if __name__ == "__main__":
    ...
    gui = GUI(10, [16, 22, 23, 24, 26, 27, 32, 33, 34, 37, 38, 48, 58, 62, 63, 64, 67, 68, 72, 73, 74, 76, 77, 86], screenHeight=800)
    gui.mainLoop()